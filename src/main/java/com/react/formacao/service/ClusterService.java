...
public class ClusterService {
    ...
    private void preecherGrupo(ArrayList<ArrayList<Aluno>> grupos, int estimativa) throws Exception {
        ...
        while (AlunosSemGrupo.size() > 0) {
            for (int i = 0; i < grupos.size(); i++) {
                ...
                AlunoClassificacaoEnum aluno = grupos.get(i).get(0).getTipoSocial();
                ArrayList<Aluno> alunos = grupos.get(i);
                if (aluno == AlunoClassificacaoEnum.LIDER) {
                    preencherCabeca(alunos, this.Fleumatico, this.Melancolico, this.Ativo, AlunoClassificacaoEnum.AMORFO, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.SOCIAL) {
                    preencherCabeca(alunos, this.Amorfo, this.Ativo, this.Fleumatico, AlunoClassificacaoEnum.MELANCOLICO, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.ATIVO) {
                    preencherCabeca(alunos, this.Instavel, this.Social, this.Lider, AlunoClassificacaoEnum.APATICO, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.MELANCOLICO) {
                    preencherCabeca(alunos, this.Lider, this.Instavel, this.Apatico, AlunoClassificacaoEnum.SOCIAL, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.INSTAVEL) {
                    preencherCabeca(alunos, this.Melancolico, this.Amorfo, this.Ativo, AlunoClassificacaoEnum.FLEUMATICO, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.AMORFO) {
                    preencherCabeca(alunos, this.Instavel, this.Apatico, this.Social, AlunoClassificacaoEnum.LIDER, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.APATICO) {
                    preencherCabeca(alunos, this.Melancolico, this.Amorfo, this.Fleumatico, AlunoClassificacaoEnum.ATIVO, AlunosSemGrupo);

                } else if (aluno == AlunoClassificacaoEnum.FLEUMATICO) {
                    preencherCabeca(alunos, this.Social, this.Lider, this.Apatico, AlunoClassificacaoEnum.INSTAVEL, AlunosSemGrupo);
                } else {
                    throw new Exception("nao identificado");
                }
            }
        }
    }
    ...
    public void organizarGrupo(ArrayList<ArrayList<Aluno>> grupos, AlunoRepository alunoRepository) { //salvar no banco
        for (int i = 0; i < grupos.size(); i++) {
